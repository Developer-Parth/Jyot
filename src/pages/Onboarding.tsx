  ...
  const steps = [
    {
      ...
      icon: <div className="w-32 h-32 rounded-full bg-orange-100 flex items-center justify-center mb-8 mx-auto shadow-inner"><span className="text-6xl">🕉️</span></div>
    },
    {
      ...
      icon: <div className="w-32 h-32 rounded-full bg-rose-100 flex items-center justify-center mb-8 mx-auto shadow-inner"><Heart className="w-16 h-16 text-rose-500" /></div>
    },
    {
      title: "Core Features",
      ...
      content: (
        <ul className="text-left space-y-4 w-full max-w-xs mx-auto text-stone-600">
          <li className="flex items-center gap-3"><span className="w-8 h-8 rounded-full bg-orange-100 flex items-center justify-center text-orange-600">📅</span> Daily Panchang</li>
          <li className="flex items-center gap-3"><span className="w-8 h-8 rounded-full bg-orange-100 flex items-center justify-center text-orange-600">📿</span> Digital Jaap Mala</li>
          <li className="flex items-center gap-3"><span className="w-8 h-8 rounded-full bg-orange-100 flex items-center justify-center text-orange-600">✨</span> AI Palmistry</li>
          <li className="flex items-center gap-3"><span className="w-8 h-8 rounded-full bg-orange-100 flex items-center justify-center text-orange-600">📖</span> Puja Vidhi Library</li>
        </ul>
      )
    },
    {
      ...
      content: (
        <div className="space-y-4 w-full max-w-xs mx-auto text-left">
          <div>
            ...
            <div className="relative">
              ...
              <input 
                ...
                className="w-full pl-10 pr-4 py-3 rounded-xl border border-stone-200 focus:ring-2 focus:ring-orange-500 focus:border-orange-500 outline-none transition-all"
          ...
          <div>
            ...
            <select 
              className="w-full px-4 py-3 rounded-xl border border-stone-200 focus:ring-2 focus:ring-orange-500 focus:border-orange-500 outline-none transition-all bg-white"
      ...
      <div className="p-8 pb-12">
        <div className="flex justify-center gap-2 mb-8">
          {steps.map((_, i) => (
            <div 
              key={i} 
              className={`h-1.5 rounded-full transition-all duration-300 ${i === step ? 'w-8 bg-orange-500' : 'w-2 bg-stone-200'}`}
            />
          ))}
        </div>
        <button
          onClick={handleNext}
          className="w-full py-4 rounded-2xl bg-orange-600 text-white font-medium text-lg shadow-lg shadow-orange-600/20 flex items-center justify-center gap-2 hover:bg-orange-700 transition-colors"
        >
          {step === steps.length - 1 ? (
